...
export const NODE_DOCUMENTATION: Record<string, NodeDocumentation> = {
  ...
  voxelSolver: {
    tips: [
      "Voxelize geometry into a cubic 3D occupancy grid (density 0/1)",
      "Solid mode fills the interior; Surface mode returns a shell (use thickness to widen)",
      "Higher resolution produces tighter detail but is more expensive",
      "Extract a renderable mesh with Extract Isosurface (isoValue 0.5 for occupancy)",
      "Use the output fillRatio and occupiedCells for quick sanity checks",
    ],
    examples: [
      "Voxelize a box at resolution 16 (solid), then Extract Isosurface at isoValue 0.5",
      "Generate a surface shell: mode=surface, thickness=2 → fast faceted proxy mesh",
      "Reuse a pre-sized grid by wiring a Voxelize Geometry output into voxelGrid",
    ],
    pitfalls: [
    ...
    bestPractices: [
      "Start with resolution 12-20 for interactive workflows",
      "Use Surface mode when you only need a proxy boundary mesh",
      "When chaining into Extract Isosurface, set isoValue to 0.5 for occupancy grids",
    ],
    relatedNodes: ["voxelizeGeometry", "extractIsosurface", "topologySolver"],
    workflowNotes: "The Voxel Solver (Ἐπιλύτης Φογκελ) voxelizes a geometry domain into a cubic density grid. Use it when you want a consistent voxel representation for downstream operations (isosurface extraction, occupancy analysis, or as a scaffold for optimization).",
  },
  chemistrySolver: {
    tips: [
